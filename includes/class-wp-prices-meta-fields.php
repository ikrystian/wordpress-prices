    ...
    public function add_margin_category_field() {
        ...
        if (!empty($current_value) && isset($margins[$current_value])) {
            ...
            if ($product && $product->get_price()) {
                ...
                $price_without_margin = $price_with_margin / (1 + ($margin_percentage / 100));
                
                echo '<p class="form-field wp-prices-info">';
                echo '<strong>' . __('Informacje o marży:', 'wordpress-prices') . '</strong><br>';
                echo sprintf(__('Marża: %s%%', 'wordpress-prices'), $margin_percentage) . '<br>';
