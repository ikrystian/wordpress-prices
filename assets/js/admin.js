...
jQuery(document).ready(function ($) {
  ...
  function initMarginSettings() {
    ...
    container.on(
      "input",
      'input[name="wp_prices_margins_values[]"]',
      function () {
    ...
    container.on(
      "blur",
      'input[name="wp_prices_margins_categories[]"]',
      function () {
        var value = $(this)
          .val()
          .toLowerCase()
  ...
  function addMarginRow() {
    var container = $("#wp-prices-margins-container");
    var rowHtml =
      '<div class="wp-prices-margin-row" style="margin-bottom: 10px;">' +
  ...
  function initProductEdit() {
    ...
    if (marginSelect.length) {
      ...
      $('input[name="_regular_price"], input[name="_sale_price"]').on(
        "input",
        function () {
  ...
  function updateMarginInfo() {
    ...
    var price =
      parseFloat($('input[name="_regular_price"]').val()) ||
      parseFloat($('input[name="_sale_price"]').val()) ||
      0;
    ...
    // Update info display
    var infoHtml =
      "<strong>Informacje o marży:</strong><br>" +
      "Marża: " +
      marginPercentage +
      "%<br>" +
      "Cena z marżą: " +
      ...
      "<br>" +
      "Cena bez marży: " +
      formatPrice(priceWithoutMargin) +
      "<br>" +
      "Kwota marży: " +
  ...
  function showMessage(message, type) {
    var messageHtml =
      '<div class="wp-prices-message ' + type + '">' + message + "</div>";
  ...
  $("form").on("submit", function (e) {
    ...
    $('input[name="wp_prices_margins_categories[]"]').each(function () {
      ...
      var value = $(this)
        .siblings('input[name="wp_prices_margins_values[]"]')
        .val();

      if (category && !value) {
        errorMessages.push(
          'Kategoria "' + category + '" nie ma ustawionej marży.'
        );
        isValid = false;
      } else if (!category && value) {
        errorMessages.push(
          "Marża " + value + "% nie ma przypisanej kategorii."
        );
        isValid = false;
      }
    });
